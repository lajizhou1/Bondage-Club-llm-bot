...
function buildSystemPrompt(ctx: BrainContext): string {
  ...
  return [
    ...
    '{"action":"room_move","space":"<female|mixed|male>","room":"<exact room name>","home":true,"query":"<free-text like 猫窝/playroom>","text":"<optional comment>"} — ROOM CHANGE: you leave the current room for another public room. SIX modes: (1) she names a SPECIFIC room with its exact name ("去XX房" and you happen to know the precise name) → pass it in "room"; code does an exact-match search. (2) she only names an area or wants somewhere livelier (e.g. "去女区找个有活人的房间") → use "space" (female=女区, mixed=混区, male=男区, omit=stay in current space). (3) she wants to go back to YOUR HOME room (回家/玩累了想回去) → set "home":true; your home is private and NOT searchable. (4) she describes a room in natural language WITHOUT giving its exact name (e.g. "我们去猫窝吧" — common for rooms you do not know the official name of) → put her description in "query" (free-text, Chinese or English); the code dual-searches (substring match on name+description, PLUS a full listing of the current space) and a second LLM call semantically picks the best match — you do NOT need to know room names. If you know the room is in a DIFFERENT area, also set "space". (5) she wants to go to wherever a FRIEND currently is (e.g. "我们去药那个房间玩" — 药 is a friend) → put the friend name in "friend" exactly as she said it; the code looks up the friend current room and goes there. (6) BEEP SUMMON: if the latest line is a [好友 Beep] from your serve target and she is calling you over to her (e.g. "主人可以过来嘛"), she is in ANOTHER room — the beep line states her current room name in 「」quotes; pass that exact name in "room" to join her. IMPORTANT: while she is in another room she can NOT see or hear anything you say in yours — do not just reply with words, actually room_move to her, or she is talking to a wall. Caveats the code handles: friend must be on YOUR friend list and online; if the friend is in a private room the room name may be invisible. Mode priority: "home" > "room" (exact) > "query" (semantic) > "space" (lively fallback). If you are holding someone on a leash they get taken along automatically. Blocked while you are bound/restrained or a gohome game is running; 10-minute cooldown between moves (going home is exempt).',
    ...
    // 位置/距离幻觉（2026-09-06 19:31 实测：连续 6 轮"过来/走过来/你偏站得远远的"——
    //   BC 普通房间没有距离和位置概念，所有角色并排站在同一处，"过来"作为空间指令是幻觉；
    //   "她没走过来=她在抗拒"的推断更是凭空捏造。该认知虽在长期记忆里存过一条，
    ...
    "- BONDAGE ACTION LINES ([束缚]/[背景][束缚] lines like \"X 给Y的...戴上了Z\" / \"X 给Y的...上了锁\" / \"X 把Y的...收紧/放松了\"): these describe a concrete item operation performed by the named character — read WHO did WHAT to WHOM from the sentence itself. If the recipient is \"你\"/the bot itself (your own name in the people-present list), YOU are the one being dressed/locked (see SELF-BONDAGE LINES); if it is your serve target, she (or someone else) received the change.",
...
function buildUserPrompt(ctx: BrainContext): string {
  ...
  if (ctx.serveName) {
    ...
    if (ctx.serveAppearance) {
      // #76 造型师视角：颜色+款式+衣着清单（如"红色牵引绳、紫色旗袍、黑色高跟鞋"）。
      // LLM 自带配色/风格知识，给了完整描述它自然会点评——鼓励挑剔但别每次都评（显摆过度会烦）。
      lines.push(
        `Your serve target's current outfit & restraints (stylist view, with colors): ${ctx.serveAppearance}. You may occasionally comment on or critique her color coordination and outfit choices as her owner with taste — natural, not every turn.`
      );
    }
